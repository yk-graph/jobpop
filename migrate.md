...
```json
{
  ...
  "tasks": {
    "build": {
      "dependsOn": ["^build"],
      "outputs": [".next/**", "dist/**", "src/generated/**"]
    },
    "dev": {
      "cache": false,
      "persistent": true
...
| 設定                    | 値                                              | 意味                                                   |
| ----------------------- | ----------------------------------------------- | ------------------------------------------------------ |
| `globalEnv`             | 環境変数の配列                                  | ビルド時に必要な環境変数を宣言（Vercelデプロイに必須） |
| `dependsOn: ["^build"]` | `^` = 依存先を先に                              | web が database に依存しているなら、database を先にビルド |
| `outputs`               | `[".next/**", "dist/**", "src/generated/**"]`   | キャッシュ対象のファイル                               |
| `cache: false`          | キャッシュしない                                | dev サーバーは毎回起動が必要なのでキャッシュ無効       |
| `persistent: true`      | 常駐プロセス                                    | dev サーバーのように終了しないプロセス                 |

### globalEnv について

**なぜ必要か**：Turborepoはデフォルトで環境変数をビルドタスクに渡さない。`globalEnv`で明示的に宣言することで：
1. ビルド時に環境変数が利用可能になる
2. 環境変数が変更された時にキャッシュが無効化される

```
Vercel/CI環境変数             turbo.json
─────────────────────         ────────────────────
RESEND_API_KEY=xxx        →   globalEnv に宣言
                              ↓
                              ビルド時に利用可能
```
...
**役割**：ビルドで生成されるファイルをキャッシュ対象として指定

```
outputs の指定                  実際のキャッシュ対象
──────────────────────          ────────────────────────────────────
.next/**                    →   apps/web/.next/**（Next.jsビルド成果物）
src/generated/**            →   packages/database/src/generated/**（Prismaクライアント）
dist/**                     →   (各パッケージ)/dist/**（TypeScriptコンパイル出力）
```

**パスの解釈**：
- outputs はグローバル設定だが、タスク実行時は各パッケージのディレクトリを基準に解釈される
- 例：`@jobpop/web:build` 実行時、作業ディレクトリは `apps/web/` なので `.next/**` は `apps/web/.next/**` を指す

**キャッシュの仕組み**：
```
1回目のビルド
─────────────
タスク実行 → 結果を outputs に出力 → Turboがキャッシュに保存

2回目のビルド（コード変更なし）
─────────────
Turbo: キャッシュヒット → タスクをスキップ → キャッシュから outputs を復元
```
...
モノレポでは、Vercelに「どのディレクトリをデプロイするか」を指定する必要がある。

**設定方法**：
1. Vercelプロジェクト設定 → **Settings** → **General**
2. **Root Directory** を `apps/web` に設定

```
jobpop/                  ← Vercelがデフォルトで見る場所
├── package.json         ← "next"がない（Turborepoのルート）
└── apps/
    └── web/
        └── package.json ← "next"がある（本来デプロイすべき場所）
```

### Prisma generate の自動実行

**問題**：ローカルでは`prisma generate`済みだが、Vercelは毎回クリーンビルドのため、生成されたクライアントが存在しない。

```
Module not found: Can't resolve './generated/client'
```

**解決策**：`packages/database/package.json`に`build`スクリプトを追加
...
pnpm build 実行
      ↓
Turboが依存関係を解析（turbo.json: dependsOn: ["^build"]）
      ↓
@jobpop/database:build が先に実行
      ↓
prisma generate → src/generated/client が作成される
      ↓
@jobpop/web:build が実行
      ↓
generated/client が存在するのでビルド成功
```

### 環境変数の設定

**問題**：Turborepoはデフォルトで環境変数をビルドに渡さない。

```
Error: Missing API key. Pass it to the constructor `new Resend("re_123")`
```

**解決策**：`turbo.json`に`globalEnv`を追加（上記「turbo.json」セクション参照）
...
| `AUTH_SECRET` | 認証 |
| `APP_URL` | アプリのURL |
| `AWS_ACCESS_KEY_ID` | S3アクセス |
| `AWS_SECRET_ACCESS_KEY` | S3アクセス |
| `AWS_REGION` | S3リージョン |
| `AWS_S3_BUCKET_NAME` | S3バケット |
| `GOOGLE_CLIENT_ID` | Google OAuth |
| `GOOGLE_CLIENT_SECRET` | Google OAuth |
| `PUBLIC_GOOGLE_MAPS_API_KEY` | Google Maps（クライアント側） |
| `PRIVATE_GOOGLE_MAPS_API_KEY` | Google Maps（サーバー側） |

### Node.jsバージョンの設定

ローカル（Volta）とVercelでNode.jsバージョンを合わせる。

**Vercelでの設定**：
- **Settings** → **General** → **Node.js Version** → `24.x`

**確認**：ルートの`package.json`
```json
{
  "volta": {
    "node": "24.11.0"
